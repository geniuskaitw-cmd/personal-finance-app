...
// ?–å??‡å??¥æ??€?¨é€±ï??±ä?ï½é€±æ—¥ï¼‰ç??¥æ????
function getWeekRange(dateStr: string): string[] {
  const d = new Date(dateStr);
  const day = d.getDay() === 0 ? 7 : d.getDay(); // Sunday=0 èª¿æ•´??7
...
// Icon mappingï¼šå?é¡å?ç¨???Icon ?ƒä»¶
function CategoryIcon({ category }: { category: string | null }) {
  const cat = category ?? '';

  if (cat === 'é¤é£²é£Ÿå?') return <Utensils className="w-8 h-8" />;
  if (cat === 'äº¤é€?) return <Car className="w-8 h-8" />;
  if (cat === '?¥ç”¨??) return <ShoppingBag className="w-8 h-8" />;
  if (cat === 'å¨›æ?') return <Gamepad2 className="w-8 h-8" />;
  if (cat === '?«ç?') return <Stethoscope className="w-8 h-8" />;
  if (cat === '?™è‚²') return <GraduationCap className="w-8 h-8" />;
  if (cat === 'ä½æˆ¿') return <Home className="w-8 h-8" />;
  if (cat === 'æ°´é›»?¦æ–¯') return <Lamp className="w-8 h-8" />;
  if (cat === '?šè?ç¶²è·¯') return <Wifi className="w-8 h-8" />;
  if (cat === '?…è?') return <Plane className="w-8 h-8" />;
  if (cat === '?é£¾è¡?‰©') return <Shirt className="w-8 h-8" />;
  if (cat === '?œè²»') return <Gift className="w-8 h-8" />;
...
function TodayPageContent() {
  ...
  const [showActionMenu, setShowActionMenu] = useState(false); // ?¯å¦é¡¯ç¤º?¸å–®
  ...
  /** -----------------------------
   * è®€??URL ?ƒæ•¸ä¸­ç? dateï¼ˆå??ˆæ?é»æ??ä??‚ï?
   * ----------------------------- */
  useEffect(() => {
  ...
  /** -----------------------------
   * ?±ç¨± mapï¼šå? Supabase è®€??user_names
   * ----------------------------- */
  useEffect(() => {
    async function loadNameMap() {
      try {
        ...
        console.warn('è®€?–æš±ç¨±ç™¼?ŸéŒ¯èª?, e);
      }
    }
  ...
  async function doDelete() {
    ...
    if (error) {
      ...
      alert('?ªé™¤å¤±æ?ï¼šå¯?½æ˜¯æ¬Šé?ä¸è¶³ (RLS) ?–æ‰¾ä¸åˆ°è©²ç?è³‡æ??‚è?æª¢æŸ¥ Supabase ??Delete Policy??);
    } else {
      // ?æ–°è®€?–è???
      loadData();
    }
  ...
  async function doUpdate() {
    ...
    const { error, data } = await supabase
      ...
      .update({
        ...
        // message æ¬„ä??šå¸¸??note ?Œæ­¥ï¼Œæ??ªç”¨ noteï¼Œé€™è£¡ä¿æ??Œæ­¥
    ...
    if (error) {
      ...
      alert('?´æ–°å¤±æ?ï¼šå¯?½æ˜¯æ¬Šé?ä¸è¶³ (RLS) ?–æ‰¾ä¸åˆ°è©²ç?è³‡æ??‚è?æª¢æŸ¥ Supabase ??Update Policy??);
    } else {
      loadData();
    }
    closeAllModals();
  ...
  /** -----------------------------
   * å·¦å³?‡æ??ŒåŸºæº–æ—¥?Ÿã€ï?ä¾æ¨¡å¼ï??????ˆï???
   * ----------------------------- */
  function handlePrev() {
    if (mode === 'day') {
    ...
    <div className="min-h-screen flex flex-col bg-md-background">
      ...
      <div className="p-4 pb-20 max-w-4xl mx-auto w-full space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <option value="time-desc">?‚é?ï¼šæ–° ????/option>
            <option value="time-asc">?‚é?ï¼šè? ????/option>
            <option value="amount-desc">?‘é?ï¼šå¤§ ??å°?/option>
            <option value="amount-asc">?‘é?ï¼šå? ??å¤?/option>
          </select>
        </div>

        {/* Loading state */}
        {loading && (
          <p className="text-center mt-4 text-md-on-surface-variant">è¼‰å…¥ä¸?..</p>
        )}
        ...
        {!loading && data.length === 0 && (
          <section className="flex flex-col items-center justify-center py-20 px-4 space-y-8 glass-card rounded-2xl border border-md-outline-variant/5 min-h-[400px]">
            ...
            <div className="text-center space-y-3">
              <h3 className="font-[family-name:var(--font-headline)] text-2xl font-semibold text-md-on-surface tracking-wide">æ²’æ?è³‡æ?</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-6">
          <div className="w-full max-w-sm glass-card rounded-2xl p-6 shadow-xl">
            ...
            <p className="mb-6 text-md-on-surface-variant">
              æ­¤å?ä½œç„¡æ³•å¾©?Ÿï?è©²ç?è¨˜å¸³ç´€?„å??ƒæ°¸ä¹…åˆª?¤ã€?
            </p>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm sm:p-6">
          <div className="w-full max-w-md sm:rounded-2xl rounded-t-2xl p-5 shadow-xl h-[85vh] sm:h-auto flex flex-col glass-card">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-xl font-bold text-md-on-surface">ç·¨è¼¯è¨˜å¸³</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-md-on-surface-variant">?‘é?</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-md-on-surface-variant">?…ç›® (?™è¨»)</label>
                <input
                  type="text"
                  value={editForm.note}
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-md-on-surface-variant">é¡åˆ¥</label>
...
export default function TodayPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-md-background"><p className="text-md-on-surface-variant">è¼‰å…¥ä¸?..</p></div>}>
      <TodayPageContent />
    </Suspense>
  );
}
